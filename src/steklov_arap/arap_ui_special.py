...
def main() -> None:
    ...
    components_path = Path(args.mesh_path).parent / "golem_components.json"
    with components_path.open("r", encoding="utf-8") as f:
        golem_components = json.load(f)
    for name, vertex_indices in golem_components.items():
        if name in {"head", "body", "arm_right"} or name.startswith("finger_right_"):
            locked_vertices.update(
                int(i) for i in vertex_indices if 0 <= int(i) < n_vertices
            )
